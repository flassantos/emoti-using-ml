...
if __name__ == '__main__':
    ...
    param_grid = {
        ...
        'LogisticRegression': {
            'clf': [LogisticRegression(random_state=42, solver='saga', max_iter=1000, n_jobs=-1)],
            'clf__C': [0.1, 1.0, 10.0]
        },
        'KNeighbors': {
        ...
        # 'SVM': {
        ##     'clf': [SVC(degree=2, random_state=42)],
        ##     # 'clf': [OneVsRestClassifier(CalibratedClassifierCV(SVC(random_state=42), method='sigmoid'))],
        ##     'clf__C': [0.1, 1, 10],
        ##     # 'clf__gamma': ['scale', 'auto']
        ## },
        'LinearSVC': {
            'clf': [CalibratedClassifierCV(LinearSVC(dual='auto', random_state=42))],
    ...
    for network in networks + ['ALL']:
        ...
        # Define y
        y = df['y_binary']  # Use y_binary as label
        ...
        scoring = {
            ...
            'auprc': make_scorer(average_precision_score, average='macro', needs_proba=True),
